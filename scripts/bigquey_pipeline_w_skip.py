...
class BQPipeline:
    def __init__(self, project_id, dataset_id, bucket_name, 
                 credential_path, N, prefix='training_data', dN = 100, skip_intermediate_load=False):
        ...
        self.skip_intermediate_load = skip_intermediate_load
    ...
    def merge_and_create_final_bq_table(self, main_table_id):
        ...
        for N1, N2 in zip(range(0, self.N, self.dN), range(self.dN, self.N + self.dN, self.dN)):
            ...
            if not blobs_list:
                # print('empty', N1, N2)
                logging.info('Empty: %s %s', N1, N2)
                continue
            #print(N1, N2)
            logging.info(' %s %s', N1, N2)
            parquet_files = [blob.name for blob in blobs_list if blob.name.endswith(".parquet")]
            table_names = []

            if not self.skip_intermediate_load:
                ...
                merge_sql = f"CREATE OR REPLACE TABLE `{self.dataset_id}.{interim_table_id}` AS " + " UNION DISTINCT ".join(union_queries)
