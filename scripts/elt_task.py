...
from bigquey_pipeline_w_skip import BQPipeline
import os
...
def main_elt_task():
    # Configure logging
    logging.basicConfig(filename='elt_task.log', level=logging.INFO,  
                        format='%(asctime)s - %(levelname)s - %(message)s')
    try:

        # Set Google credentials
        set_google_credentials(gc_key)
        logging.info('1. Done: Credentials')
        ...
        logging.info('Done: BQPipeline created')
        pipeline.merge_and_create_final_bq_table(main_table_id)    
        logging.info('Done: BQPipeline merged!!!!')
